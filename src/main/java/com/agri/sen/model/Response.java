...
public class Response<T> {
    ...
    public static <T> Response<T> invalidToken() {  // ✅ AJOUTEZ CETTE MÉTHODE
        Response<T> response = new Response<>();
        response.setStatus(Status.INVALID_TOKEN);
        return response;
    }

    public static <T> Response<T> invalidUrl() {
        Response<T> response = new Response<>();
        response.setStatus(Status.INVALID_URL);
        return response;
    }
    ...
    public enum Status {
        ...
        TOKEN_EXPIRED, FIRST_CONNECTION, DISABLED_ACCOUNT, INVALID_TOKEN, INVALID_URL, UNSUPPORTED_MEDIA_TYPE,
        DIRECTORY_NOT_FOUND, CREATED, DELETED
    }
